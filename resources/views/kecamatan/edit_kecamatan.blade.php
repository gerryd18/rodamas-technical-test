...
@section('content')
    ...
    <div class="container flex items-center justify-center">
        <div class="w-1/2 p-5 rounded-xl bg-blue-200">
            <form class="w-full max-w-lg mx-auto" action="{{route('update.kecamatan',$kecamatan->id)}}" method="POST">
                @csrf
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="kode" name="kode" type="text" placeholder="Kode" value="{{$kecamatan->kode}}">
                    </div>
                </div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3 mb-6 md:mb-0">
                    ...
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="nama_kecamatan" name="nama_kecamatan" type="text" placeholder="Nama Kecamatan" value="{{$kecamatan->nama_kecamatan}}">
                    </div>
                </div>
                <div class="flex items-center mb-6">
                    <div class="w-full px-3">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                         @if($kecamatan->isActive == 1)
                            <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" name="isActive" checked>
                        @elseif($kecamatan->isActive == 0)
